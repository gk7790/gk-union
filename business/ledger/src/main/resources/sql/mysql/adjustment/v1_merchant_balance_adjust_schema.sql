...
CREATE TABLE `merchant_balance_adjust_order` (
  ...
  `adjust_order_no` varchar(128) NOT NULL COMMENT '余额调整单号',
  `adjust_type` varchar(32) NOT NULL COMMENT '调整类型: RECHARGE/DEDUCT/REVERSE/SUPPLEMENT',
  `source_type` varchar(32) NOT NULL DEFAULT 'MANUAL' COMMENT '来源: MANUAL/SYSTEM/API',
  `currency` varchar(16) NOT NULL COMMENT '币种',
  `amount` decimal(24, 8) NOT NULL COMMENT '调整金额，永远为正数',
  `status` varchar(32) NOT NULL DEFAULT 'CREATED' COMMENT '状态: CREATED/POSTED/FAILED',
  `reason` varchar(512) NULL DEFAULT NULL COMMENT '调整原因',
  `related_order_no` varchar(128) NULL DEFAULT NULL COMMENT '关联业务单号',
  `reverse_of_journal_no` varchar(64) NULL DEFAULT NULL COMMENT '冲正来源凭证号',
  `ledger_journal_no` varchar(64) NULL DEFAULT NULL COMMENT '账本凭证号',
  `trace_id` varchar(128) NULL DEFAULT NULL COMMENT '链路追踪ID',
  `posted_at` datetime(3) NULL DEFAULT NULL COMMENT '入账时间',
  `operator_type` varchar(32) NOT NULL DEFAULT 'ADMIN' COMMENT '操作方类型: ADMIN/SYSTEM/API',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='商户余额调整单';
