...
public class TenantCurrencyServiceImpl extends CrudServiceImpl<TenantCurrencyDao, TenantCurrencyEntity, TenantCurrencyDTO>
    ...
    @Override
    public void sync(Long tenantId, List<String> currencies) {
        if (tenantId == null || currencies == null || currencies.isEmpty()) return;
        QueryWrapper<TenantCurrencyEntity> wrapper = new QueryWrapper<>();
        wrapper.eq("tenant_id", tenantId);
        List<TenantCurrencyEntity> existing = baseDao.selectList(wrapper);
        Map<String, TenantCurrencyEntity> existingMap = existing.stream()
                .filter(item -> StrUtil.isNotBlank(item.getCurrency()))
                .collect(Collectors.toMap(TenantCurrencyEntity::getCurrency, item -> item, (a, b) -> a));
        for (int i = 0; i < currencies.size(); i++) {
            String currency = currencies.get(i);
            TenantCurrencyEntity current = existingMap.get(currency);
            int sort = (i + 1) * 10;
            if (current == null) {
                current = new TenantCurrencyEntity();
                current.setTenantId(tenantId);
                current.setCurrency(currency);
                current.setStatus(StatusEnum.NORMAL.code());
                current.setSort(sort);
                baseDao.insert(current);
            } else {
                baseDao.update(null, new UpdateWrapper<TenantCurrencyEntity>()
                        .set("status", StatusEnum.NORMAL.code()).set("sort", sort).eq("id", current.getId()));
            }
        }
        List<String> removed = existingMap.keySet().stream().filter(item -> !currencies.contains(item)).toList();
        if (!removed.isEmpty()) {
            baseDao.update(null, new UpdateWrapper<TenantCurrencyEntity>()
                    .set("status", StatusEnum.STOP.code()).eq("tenant_id", tenantId).in("currency", removed));
        }
    }

    @Override
    public Map<Long, List<String>> findCurrencies(List<Long> tenantIds) {
        if (tenantIds == null || tenantIds.isEmpty()) return Map.of();
        QueryWrapper<TenantCurrencyEntity> wrapper = new QueryWrapper<>();
        wrapper.in("tenant_id", tenantIds).in("status", StatusEnum.defaultStatus())
                .orderByAsc("tenant_id", "sort", "currency");
        return baseDao.selectList(wrapper).stream().collect(Collectors.groupingBy(
                TenantCurrencyEntity::getTenantId,
                Collectors.mapping(TenantCurrencyEntity::getCurrency,
