...
/**
 * PSP 回调请求对象工厂 * <p>
 * 负责把原{@link HttpServletRequest}、路径中PSP 编码和业务类型封装成统一 * {@link PspCallbackRequest}，供后续适配器验签、解析和落日志使用 */
@Component
...
public class PspCallbackRequestFactory {
    ...
    private DynMap params(HttpServletRequest request, String rawBody) {
        ...
        // 先放 query string，再body；同名字段以 body 中的值为准        appendFormParams(params, request.getQueryString());
        ...
        if (body.startsWith("{")) {
            try {
                ...
                // body 不是合法 JSON 时保留已解析query 参数，后续由适配器或校验流程报错
                                return params;
            }
            return params;
        }
    ...
    /**
     * 获取客户IP     * <p>
     * 优先取代理头中的首个 IP，最后回退Servlet 容器提供remoteAddr     */
