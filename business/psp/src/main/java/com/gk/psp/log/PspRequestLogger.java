...
/**
 * PSP 出站请求日志记录器 * <p>
 * 平台主动请求 PSP 的场景包括：代收下单、代付提交、代收查单、代付查单 * 本类负责把这些请求的 URL、方法、请求体、响应体、耗时、成功状态和错误信息写入
 * {@code psp_request_log}，便于排查上游调用、对账和异常补偿 * <p>
...
public class PspRequestLogger {
    ...
    /**
     * 记录代收下单适配器正常返回的请求日志     * <p>
     * 注意：适配器正常返回不等于 PSP 业务成功，result.success 会被写入 success 字段     */
    public void paySubmitSuccess(PspOrderRequest order, PspRouteResult route, PspPayDispatchResult result, long costMs) {
    ...
    /**
     * 记录代收下单过程中抛出异常的请求日志     * <p>
     * 例如适配器缺失、网络异常、签名构造异常等，这类场景没有完PSP 响应     */
    public void paySubmitFailed(PspOrderRequest order, PspRouteResult route, Throwable throwable, long costMs) {
        PspRequestLogEntity entity = baseEntity(order, route, (PspPayDispatchResult) null);
    ...
    /**
     * 记录代付提交适配器正常返回的请求日志     * <p>
     * 代付通常先冻结商户余额，再提PSP；该日志可用于追踪冻结后是否成功请求上游     */
    public void payoutSubmitSuccess(PspOrderRequest order, PspRouteResult route, PspPayoutDispatchResult result, long costMs) {
        PspRequestLogEntity entity = baseEntity(order, route, result);
    ...
    /**
     * 记录代付提交过程中抛出异常的请求日志     * <p>
     * 上层会根据异常决定订单失败和冻结释放，本方法只负责保存上游调用失败证据     */
    public void payoutSubmitFailed(PspOrderRequest order, PspRouteResult route, Throwable throwable, long costMs) {
        PspRequestLogEntity entity = baseEntity(order, route, (PspPayoutDispatchResult) null);
    ...
    public void payQuerySuccess(PspOrderRequest order, PspRouteResult route, PspOrderQueryResult result, long costMs) {
        PspRequestLogEntity entity = baseEntity(order, route, (PspPayDispatchResult) null);
    ...
    /**
     * 记录代收主动查单异常日志     */
    public void payQueryFailed(PspOrderRequest order, PspRouteResult route, Throwable throwable, long costMs) {
        PspRequestLogEntity entity = baseEntity(order, route, (PspPayDispatchResult) null);
    ...
    /**
     * 记录代付主动查单正常返回的请求日志     * <p>
     * 查单结果后续会走PSP 回调类似的状态更新、账务扣冻结或解冻逻辑     */
    public void payoutQuerySuccess(PspOrderRequest order, PspRouteResult route, PspOrderQueryResult result, long costMs) {
        PspRequestLogEntity entity = baseEntity(order, route, (PspPayoutDispatchResult) null);
    ...
    /**
     * 记录代付主动查单异常日志     */
    public void payoutQueryFailed(PspOrderRequest order, PspRouteResult route, Throwable throwable, long costMs) {
        PspRequestLogEntity entity = baseEntity(order, route, (PspPayoutDispatchResult) null);
    ...
    private PspRequestLogEntity baseEntity(PspOrderRequest order, PspRouteResult route, PspPayDispatchResult result) {
    ...
    /**
     * 构建代付 PSP 请求日志基础字段     */
    private PspRequestLogEntity baseEntity(PspOrderRequest order, PspRouteResult route, PspPayoutDispatchResult result) {
        PspRequestLogEntity entity = new PspRequestLogEntity();
        entity.setTenantId(order == null ? null : order.getTenantId());
        entity.setMerchantId(order == null ? null : order.getMerchantId());
        entity.setPspId(route == null ? null : route.getPspId());
    ...
    private Map<String, Object> payoutRequestSnapshot(PspOrderRequest order, PspRouteResult route) {
        Map<String, Object> snapshot = new LinkedHashMap<>();
        snapshot.put("payout_order_no", order == null ? null : order.getOrderNo());
        snapshot.put("merchant_order_no", order == null ? null : order.getMerchantOrderNo());
        snapshot.put("amount", order == null ? null : order.getAmount());
        snapshot.put("currency", order == null ? null : order.getCurrency());
        snapshot.put("method_code", order == null ? null : order.getMethodCode());
    ...
    /**
     * 生成默认代收下单 URL     * <p>
     * 适配器没有明确返requestUrl 时，PSP baseUrl 拼出一个默认地址便于排查     */
    ...
    /**
     * 脱敏请求/响应文本     * <p>
     * 对常见密钥、token、签名字段做掩码，降PSP 请求日志泄露敏感信息的风险     */
