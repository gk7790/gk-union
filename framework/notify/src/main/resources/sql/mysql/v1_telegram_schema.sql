...
CREATE TABLE `tg_account` (
  ...
  `tg_username` varchar(64) NULL DEFAULT NULL COMMENT 'Telegram用户名',
  `language_code` varchar(16) NULL DEFAULT NULL COMMENT 'Telegram语言',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Telegram账号绑定';

DROP TABLE IF EXISTS `tg_bot`;
CREATE TABLE `tg_bot` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `owner_scope` varchar(16) NOT NULL DEFAULT 'PLATFORM' COMMENT '归属: PLATFORM/TENANT',
  `tenant_id` bigint NULL DEFAULT 0 COMMENT '租户ID; TENANT必填, PLATFORM为空',
  `bot_no` varchar(64) NOT NULL COMMENT '内部机器人编号',
  `name` varchar(128) NULL DEFAULT NULL COMMENT '机器人显示名称',
  `username` varchar(64) NOT NULL COMMENT 'Bot @username',
  `bot_user_id` bigint NULL DEFAULT NULL COMMENT 'Telegram BotUserId(getMe)',
  `token_cipher` varchar(512) NOT NULL COMMENT 'BotToken密文(对称加密, 禁止明文)',
  `token_hash` varchar(128) NOT NULL COMMENT 'Token哈希(查重/校验)',
  `secret_token` varchar(128) NULL DEFAULT NULL COMMENT 'Webhook secret_token',
  `webhook_url` varchar(512) NULL DEFAULT NULL COMMENT '已设置的Webhook地址',
  `mode` varchar(16) NOT NULL DEFAULT 'WEBHOOK' COMMENT '模式: WEBHOOK/POLLING',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Telegram机器人配置';

DROP TABLE IF EXISTS `tg_chat`;
CREATE TABLE `tg_chat` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NULL DEFAULT 0 COMMENT '租户ID',
  `merchant_id` bigint NULL DEFAULT 0 COMMENT '商户ID(可选, 精细到商户)',
  `bot_id` bigint NOT NULL COMMENT '所属机器人ID, 关联tg_bot.id',
  `chat_id` bigint NOT NULL COMMENT 'Telegram ChatId(群为负数)',
  `chat_type` varchar(16) NOT NULL COMMENT '类型: PRIVATE/GROUP/SUPERGROUP/CHANNEL',
  `title` varchar(255) NULL DEFAULT NULL COMMENT '群/频道名称',
  `purpose` varchar(16) NOT NULL DEFAULT 'NOTIFY' COMMENT '用途: NOTIFY/OPS/CUSTOMER',
  `event_types` varchar(512) NULL DEFAULT NULL COMMENT '订阅事件(逗号分隔, 空=全部): SYSTEM_ERROR,SYSTEM_WARN,PAY_SUCCESS,PAYOUT_SUCCESS,PAYOUT_FAILED,RISK_ALERT',
  `lang` varchar(16) NOT NULL DEFAULT 'en-US' COMMENT '消息语言',
  `status` tinyint NOT NULL DEFAULT 1 COMMENT '状态: 1正常 2暂停 3停用',
  `remark` varchar(512) NULL DEFAULT NULL COMMENT '备注',
  `created_by` bigint NULL DEFAULT NULL COMMENT '创建人ID',
  `created_at` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Telegram会话/群组(内嵌事件订阅)';
...
CREATE TABLE `tg_message_task` (
  ...
  `task_no` varchar(128) NOT NULL COMMENT '消息任务编号',
  `biz_type` varchar(64) NULL DEFAULT NULL COMMENT '业务类型: PAYIN_ORDER/PAYOUT_ORDER等',
  `biz_no` varchar(128) NULL DEFAULT NULL COMMENT '业务编号',
  `event_type` varchar(64) NULL DEFAULT NULL COMMENT '触发事件',
  `source_event_id` varchar(64) NULL DEFAULT NULL COMMENT '来源Outbox事件ID(幂等)',
  `parse_mode` varchar(16) NOT NULL DEFAULT 'HTML' COMMENT '解析模式: HTML/MarkdownV2/NONE',
  `content` text NOT NULL COMMENT '最终发送内容，已按parse_mode渲染完成',
  `payload_json` json NULL COMMENT '扩展消息载荷，如reply_markup按钮等',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Telegram出站消息任务';
...
CREATE TABLE `tg_update_log` (
  ...
  `update_type` varchar(32) NULL DEFAULT NULL COMMENT '类型: message/callback_query等',
  `command` varchar(64) NULL DEFAULT NULL COMMENT '命令(如/balance /order)',
  `raw_json` json NOT NULL COMMENT '原始Update报文',
  `handle_status` tinyint NOT NULL DEFAULT 0 COMMENT '处理状态: 0待处理 1成功 2失败',
  `error_msg` varchar(1024) NULL DEFAULT NULL COMMENT '处理错误信息',
  `created_at` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
  PRIMARY KEY (`id`) USING BTREE,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Telegram入站更新日志';
