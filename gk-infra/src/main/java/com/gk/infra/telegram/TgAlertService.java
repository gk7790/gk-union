 ...
 * <p>
 * 业务模块优先调用 {@link #notify(TgAlertEventType, Long, Long, String, String)}，
...
public interface TgAlertService {
     ...
     * @param content    最终通知内容
     * @param traceId    链路追踪 ID，可为空
     */
    void notify(TgAlertEventType eventType, Long tenantId, Long merchantId, String content, String traceId);
    ...
    default void sysWarn(Long tenantId, Long merchantId, String content, String traceId) {
        notify(TgAlertEventType.SYSTEM_WARN, tenantId, merchantId, content, traceId);
    }
    ...
    default void sysWarnSync(Long tenantId, Long merchantId, String content, String traceId) {
        notifySync(TgAlertEventType.SYSTEM_WARN, tenantId, merchantId, content, traceId);
    }

    /** 创建风控提醒通知任务。 */
    default void riskAlert(Long tenantId, Long merchantId, String content, String traceId) {
        notify(TgAlertEventType.RISK_WARN, tenantId, merchantId, content, traceId);
    }

    /** 创建代收相关通知任务。 */
    default void paySuccess(Long tenantId, Long merchantId, String content, String traceId) {
        notify(TgAlertEventType.PAYIN_NOTICE, tenantId, merchantId, content, traceId);
    }

    /** 创建代付成功相关通知任务。 */
    default void payoutSuccess(Long tenantId, Long merchantId, String content, String traceId) {
        notify(TgAlertEventType.PAYOUT_NOTICE, tenantId, merchantId, content, traceId);
    }

    /** 创建代付失败相关通知任务。 */
    default void payoutFailed(Long tenantId, Long merchantId, String content, String traceId) {
        notify(TgAlertEventType.PAYOUT_NOTICE, tenantId, merchantId, content, traceId);
    }
}
