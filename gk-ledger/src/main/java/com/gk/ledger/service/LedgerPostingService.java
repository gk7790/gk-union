...
public interface LedgerPostingService {
    ...
    /**
     * PAY_SUCCESS �?代收成功入账至待结算账户（非可用）�?     * 借贷分录�?     * PSP/CLEARING �?settleAmount + feeAmount；历�?沙箱订单没有pspAccountId时回退 INTERNAL/CLEARING
     ...
     *      同时写一�?ledger_hold（status=HOLDING，hold_amount=remaining_amount=totalDebitAmount）�?     * 返回 journalNo + holdNo（唯一一个会�?holdNo 的方法），回填到订单�?holdNo / freezeJournalNo�?     * @param request 请求
     * @return 结果
     */
    ...
    /**
     * 代付成功（扣减冻结）
     * PSP 回调代付成功，钱真的打出去了。把之前冻的�?消�?掉，确认手续费收入�?     * 分录�?     *      商户冻结 FROZEN �?totalDebitAmount
     *      PSP 清算 CLEARING �?amount
     *      内部手续费收�?�?merchantFeeAmount
     *      对应 ledger_hold �?CONSUMED，回�?consume_journal_no 和订单的 successJournalNo。返�?journalNo�?     * @param request 请求
     * @return 结果
     */
     ...
     *      对应 ledger_hold �?RELEASED，回�?last_release_journal_no 和订单的 releaseJournalNo。返�?journalNo�?     * @param request 请求
     * @return 结果
     */
     ...
     * 5. 更新 ledger_balance，使商户可用余额增加�?     * 6. 返回入账凭证�?journalNo，供 merchant_balance_adjust_order 回填�?     * 注意：该方法只处理“充�?增加余额”场景�?     * 如果后续要统一支持充值、扣减、冲正、补账，建议使用更通用�?     * postMerchantBalanceAdjust(...)，通过 adjustType 区分 RECHARGE/DEDUCT/REVERSE/SUPPLEMENT�?     */
