 ...
 * 商户 OpenAPI 支付方式查询实现。
 *
 * <p>这里读取 payment_method 标准支付方式字典，用于告诉商户系统支持哪些 method_code。
 * 真正下单时仍由商户费率、支付计划、PSP 路由、PSP Method 和银行映射决定最终可用通道。</p>
 */
@Service
...
public class OpenPaymentMethodServiceImpl implements OpenPaymentMethodService {
     ...
     * 查询商户侧可展示的标准支付方式。
     *
     * <p>countryCode、currency、direction 会传给 PaymentMethodService，
     * 支持精确配置，也支持 payment_method 中 country/currency/direction 为空或 BOTH 的通用配置。</p>
     */
    @Override
