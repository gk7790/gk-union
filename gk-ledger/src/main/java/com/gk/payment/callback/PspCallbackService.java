...
/**
 * PSP 回调处理编排服务�? * <p>
 * 本类不负责某�?PSP 的报文解析或订单 SQL 细节，只负责把回调主链路串起来：
 * <ol>
 *   <li>组装统一请求、IP 白名单校验、选择适配器并解析报文</li>
 *   <li>定位平台订单、记录回调日志、验签与终态字段校�?/li>
 *   <li>更新订单状态、执行账务入�?解冻、创建商户通知任务</li>
 *   <li>�?PSP 协议映射 HTTP 响应（成功返�?PSP 约定文本，失败返回结构化 Result�?/li>
 * </ol>
 * <p>
...
public class PspCallbackService {
    ...
    private PspCallbackResponse handle(String pspCode, String bizType, HttpServletRequest servletRequest, String rawBody) {
        ...
        // 默认失败应答；解析成功后替换为适配器配置的 failResponse�?        String failResponse = PspCallbackResponse.DEFAULT_FAIL_BODY;
        try {
            // 数据准备阶段：IP 白名单、适配器解析、订单定位、回调日志上下文组装�?            Result<PspCallbackContext> prepared = prepare(context);
            if (prepared.isFail()) {
            ...
            // 校验阶段：验签、判断是否终态、终态时校验金额/币种/PSP编码等关键字段�?            Result<PspCallbackContext> verified = verifyAndValidate(context);
            if (verified.isFail()) {
                return finishFailed(context, verified, failResponse);
            }

            // 处理阶段：更新订单、终态入�?解冻、补写流水号、创建商户通知�?            Result<PspCallbackHandleResult> handled = process(context);
            if (handled.isFail()) {
    ...
    private Result<PspCallbackContext> prepare(PspCallbackContext context) {
        ...
        // 适配器将 PSP 原始报文解析为平台统一�?PspCallbackResult（含 orderStatus 映射）�?        Result<PspCallbackResult> parsed = BizTypeEnum.PAY_ORDER.matches(context.getBizType())
        ...
        try {
            // 解析 PSP 回调对应的平台订单�?            PspCallbackOrder order = orderResolver.resolve(context.getBizType(), context.getResult());
            context.setOrder(order);
            request.setApiSecret(order.apiSecret());
            // 先组装回调日志上下文，验�?处理结果�?finish 时异步落库�?            context.setLog(logRecorder.received(request, context.getResult(), order));
    ...
    private Result<PspCallbackContext> verifyAndValidate(PspCallbackContext context) {
        ...
        if (!context.isTerminal()) {
            // 中间态（�?PROCESSING）只更新订单进度，不做终态金额校验�?            return Result.success(context);
        }
    ...
    /**
     * 处理阶段：更新订单、终态入�?解冻、补写流水号、创建商户通知�?     * <p>
    ...
    private Result<PspCallbackHandleResult> process(PspCallbackContext context) {
        try {
            ...
            // 重复回调或未发生状态变更记�?IGNORED，仍�?PSP 返回成功应答�?            String processStatus = orderChanged
    ...
    /**
     * 阶段失败统一收尾：补写失败日志，并按错误码映�?HTTP 应答�?     */
    private PspCallbackResponse finishFailed(PspCallbackContext context, Result<?> failure, String failResponse) {
        ...
        // 验签已通过但业务校验失败（如金额不符）时，日志�?verify=SUCCESS 便于区分攻击与业务拒绝�?        String verifyStatus = verifyStatusForFailure(code);
        logRecorder.finish(
                context.getLog(),
                verifyStatus,
                PspCallbackProcessStatusEnum.FAILED.code(),
    ...
    /** 将当�?Spring 事务标记为仅回滚，用于订单已更新但后续步骤失败的补偿�?*/
