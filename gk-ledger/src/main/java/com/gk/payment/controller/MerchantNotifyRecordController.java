...
@Tag(name = "商户通知记录")
@RestController
@RequestMapping("/payment/merchant-notify-record")
@RequiredArgsConstructor
public class MerchantNotifyRecordController {
    ...
    @PreAuthorize("hasAuthority('payment:merchant-notify-record:page')")
    public R<?> page(@RequestMap DynMap params) {
        PageData<MerchantNotifyRecordDTO> page = merchantNotifyRecordService.page(params);
        return R.ok(page);
    }

    @GetMapping("{id}")
    @Operation(summary = "信息")
    @PreAuthorize("hasAuthority('payment:merchant-notify-record:info')")
    public R<?> get(@PathVariable("id") Long id) {
        return R.ok(merchantNotifyRecordService.get(id));
    }
}
