...
public class PayoutSubmitOutboxConsumer {
    ...
    private PspRouteResult routeSnapshot(PayoutOrderEntity order) {
        ...
        route.setPspBankCode(routeSnapshotValue(order.getRouteSnapshotJson(), "pspBankCode"));
        return route;
    }
    ...
    private String routeSnapshotValue(String routeSnapshotJson, String key) {
        ...
        try {
            ...
            return snapshot == null ? null : StringUtils.trimToNull(snapshot.getString(key));
        } catch (Exception ex) {
            log.warn("Parse payout route snapshot failed, key={}, err={}", key, ex.getMessage());
            return null;
        }
    }
