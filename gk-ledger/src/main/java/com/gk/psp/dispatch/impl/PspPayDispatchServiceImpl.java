...
/**
 * PSP 代收下单调度实现�? * <p>
 * 本类负责在多�?{@link PspPayAdapter} 中选择支持当前 PSP 的适配器，
...
public class PspPayDispatchServiceImpl implements PspPayDispatchService {
    ...
    public PspPayDispatchResult dispatch(PspOrderRequest order, PspRouteResult route) {
        ...
        try {
            // 根据路由结果中的 PSP 编码选择具体适配器；每个 PSP 的接口协议由适配器负责�?            PspPayAdapter adapter = adapters.stream()
            ...
            // 调用上游 PSP 创建代收订单，返回支付链接、PSP 订单号、原始响应等信息�?            PspPayDispatchResult result = adapter.createPayOrder(order, route);
            // 无论 result.success 是否�?true，只要适配器正常返回，都记录一次完整的 PSP 请求日志�?            pspRequestLogger.paySubmitSuccess(order, route, result, System.currentTimeMillis() - startMs);
            return result;
        } catch (RuntimeException ex) {
            // 适配器不存在、网络异常、签名异常等运行时错误，记录失败日志后继续抛给上层处理订单失败�?            pspRequestLogger.paySubmitFailed(order, route, ex, System.currentTimeMillis() - startMs);
            throw ex;
        }
    }
}
