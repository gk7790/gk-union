...
public class PspRouteSelectorImpl implements PspRouteSelector {
    ...
    public PspRouteResult selectPayin(PayOrderEntity order) {
        List<PspRouteRuleEntity> rules = pspRouteRuleDao.selectList(
                new QueryWrapper<PspRouteRuleEntity>()
                        ...
                        .eq("country_code", order.getCountryCode())
                        .eq("currency", order.getCurrency())
                        .eq("method_code", order.getMethodCode())
                        .eq("direction", DIRECTION_PAYIN)
                        .eq("status", STATUS_ENABLED)
                        .and(wrapper -> wrapper.isNull("merchant_id").or().eq("merchant_id", order.getMerchantId()))
                        .and(wrapper -> wrapper.isNull("merchant_app_id").or().eq("merchant_app_id", order.getMerchantAppId()))
                        .and(wrapper -> wrapper.isNull("min_amount").or().le("min_amount", order.getAmount()))
                        .and(wrapper -> wrapper.isNull("max_amount").or().ge("max_amount", order.getAmount()))
                        .orderByAsc("priority", "id")
        );

        PspRouteRuleEntity rule = rules.stream()
                .filter(this::matchesTimeWindow)
                .min(Comparator.comparingInt((PspRouteRuleEntity item) -> scopeScore(item, order))
        ...
        PspProviderEntity provider = requireProvider(rule.getPspId());
    ...
    private PspProviderEntity requireProvider(Long pspId) {
    ...
    private int scopeScore(PspRouteRuleEntity rule, PayOrderEntity order) {
        boolean merchantMatched = Objects.equals(rule.getMerchantId(), order.getMerchantId());
        boolean appMatched = Objects.equals(rule.getMerchantAppId(), order.getMerchantAppId());
