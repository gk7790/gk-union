...
@Tag(name = "系统-租户管理", description = "平台用于维护租户资料、租户开户、租户部门和租户角色选择的后台接口")
...
public class SysTenantController {
    ...
    public R<?> save(@io.swagger.v3.oas.annotations.parameters.RequestBody(description = "租户基础资料") @RequestBody SysTenantDTO dto){
        //效验数据
    ...
    @PostMapping("onboard")
    @Operation(summary = "租户开户", description = "平台一次性创建租户、默认部门、复制租户角色模板，并创建租户管理员账号及主体绑定。权限码：sys:tenant:add。")
    @PreAuthorize("hasAuthority('sys:tenant:add')")
    ...
    @Operation(summary = "租户角色列表", description = "查询指定租户可分配给租户用户的角色，只返回 role_scope=TENANT 且属于该租户的启用角色。权限码：sys:tenant:view。")
    @PreAuthorize("hasAuthority('sys:tenant:view')")
    ...
    public R<?> update(@Parameter(description = "租户ID", required = true) @PathVariable("id") Long id,
                       @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "租户基础资料") @RequestBody SysTenantDTO dto){
        //效验数据
