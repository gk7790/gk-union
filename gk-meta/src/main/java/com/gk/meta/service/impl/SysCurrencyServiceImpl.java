...
public class SysCurrencyServiceImpl extends CrudServiceImpl<SysCurrencyDao, SysCurrencyEntity, SysCurrencyDTO> implements SysCurrencyService {
    ...
    public List<SysCurrencyDTO> getOptions(DynMap params) {
        ...
        Long tenantId = ReqContextHolder.isPlatform() ? null : ReqContextHolder.getTenantId();
        if (!ReqContextHolder.isPlatform() && tenantId == null) {
            return List.of();
        }
        List<SysCurrencyDTO> options = baseDao.selectOptions(statusList, tenantId);
